function exerc1() 
  ...
  slices = [6, 10, 14, 18, 22]; #criando a variavel que armazena as fatias que vamos usar
  i = 1; #Definindo variável contadora
  figure(1, 'position',[100 0 600 600]);  #A função figure é usada para abrir a janelas gráfica
  hold on;
  for slice = slices  
    ax(i) = subplot (5,2,i);  #plotando a i-ésima imagem
    imagesc(abs(rawdata(:, :, slice))); #criando a imagem que vamos apresentar
    subplot_title = strcat("Espaço k da fatia ", {' #'}, num2str(slice));  #variável que armazena o título
    ...
    ax(i) = subplot (5,2,i+1);  #plotando a i-ézima + 1 imagem
    imagesc(abs(fft2(rawdata(:, :, slice), 128, 128))); #criando a imagem que vamos apresentar
    subplot_title = strcat("Fatia ", {' #'}, num2str(slice));  #variável que armazena o título
    title(subplot_title, "fontsize", 10); #Printando o título dessa imagem
    colormap(gray); #Permite que a imagem fique na escala de cinza
    daspect ([1 1]); #permitindo que a imagem permaneça com a proporção original
    i = i + 2; #adicionando + 2 no contador que estamos usando para plotar todos os gráficos em uma figura só
